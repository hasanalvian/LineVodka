    ...
    try:
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if msg.text == "N":
                    sendMessage(msg.to, "/nyerah")
                if msg.text == "Key":
                    sendMessage(msg.to, "Me = Kirim Kontak\nGift = Kirim Hadiah\nMid = Lihat Id\nSet = Atur CCTV\nCek = Lihat CCTV\nTag = Tag Semua Member\nM = /mulai\nN = /nyerah\nCancel = Membatalkan Undangan")
                if msg.text == "Set":
                    sendMessage(msg.to, "Star Point sudah di atur\nKetik 「Cek」 untuk melihat siapa yang ngintip")
                    try:
                ...
                if msg.text == "Cek":
                    if msg.to in wait['readPoint']:
                        ...
                        sendMessage(msg.to, "Yang udah baca: %s\nItu dia\n\nYang Cuma ngintip: %s\nGua doain jones\n\nStart Point di mulai pada jam:\n[%s]"  % (wait['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                        sendMessage(msg.to, "Star Point belum di atur\nKetik 「Set」Untuk mengatur")
                else:
                    pass
